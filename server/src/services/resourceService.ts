...
export async function catalogFacets(opts: CatalogFilter = {}) {
  ...
  // build mime/publisher/type/status dropdowns without scanning the whole catalog. each
  ...
  const [mimeTypes, pubs, prices, types, statuses] = await Promise.all([
    db
    ...
    db
      .select({ value: resources.verificationStatus, count: count() })
      .from(resources)
      .innerJoin(publishers, eq(resources.publisherId, publishers.id))
      .where(where)
      .groupBy(resources.verificationStatus)
      .orderBy(desc(count()), asc(resources.verificationStatus)),
  ]);
  ...
  return {
    ...
    statuses: statuses.map((r) => ({ value: r.value, count: Number(r.count) })),
  };
}
