...
router.get("/resources", async (req, res) => {
  ...
  const { q, type, publisher, verified, status, minPrice, maxPrice } = parsed.data;
  const filter = { q, type, publisher, verified, status, minPrice, maxPrice };
  ...
  const links = pageLinks({
    path: "/resources",
    query: { q, type, publisher, verified, status, minPrice, maxPrice, sort: parsed.data.sort },
