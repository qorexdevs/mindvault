...
| `GET` | `/resources/facets` | distinct `mimeTypes` and `publishers` plus `types`, `statuses` (`pending`, `verified`, `rejected`) and `priceRanges` (`free`, `under_1`, `1_to_5`, `5_to_20`, `20_plus`), each with a count, available under the current filter for building dropdowns and a price filter |
...
| `GET` | `/publishers/leaderboard` | creator leaderboard, ranked by `sort` (`earnings`, `sales`, `resources`, `avg_sale`, `avg_resource`, `verified`), paged with `limit`/`offset`. Total is in `X-Total-Count`; when `limit` is set, paging is in the `Link` header |
