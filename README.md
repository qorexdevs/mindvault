...
| `GET` | `/publishers/leaderboard` | creator leaderboard, ranked by `sort` (`earnings`, `sales`, `resources`, `avg_sale`), paged with `limit`/`offset`. Total is in `X-Total-Count`; when `limit` is set, paging is in the `Link` header |
